function targets=extendedFSEOF(model,biomassRxn,targetRxn,iterations,coefficient,outputFile)
...
fseof.targetUP=zeros(length(model.rxns),1);
fseof.targetDOWN=zeros(length(model.rxns),1);
    ...
    for j=1:length(fseof.results)
        if fseof.results(j,1) > 0   %Check the positive fluxes
            ...
            if i == 1   %The initial round
                rxnDirection(j,1)=1;
                fseof.targetUP(j,1)=1;
                fseof.targetDOWN(j,1)=1;
            else
                %OEs
                if (fseof.results(j,i) > fseof.results(j,i-1)) & fseof.targetUP(j,1)
                    fseof.targetUP(j,1)=1;
                else
                    fseof.targetUP(j,1)=0;
                end
                %KDs and KOs
                if (fseof.results(j,i) < fseof.results(j,i-1)) & fseof.targetDOWN(j,1)==1
                    fseof.targetDOWN(j,1)=1;
                else
                    fseof.targetDOWN(j,1)=0;
                end
...
fseof.target = logical(fseof.targetUP+fseof.targetDOWN);
%Generating output
formatSpec='%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n';
if output == 1    %Output to a file
    fid=fopen(outputFile,'w');
    fprintf(fid,formatSpec,'Slope','K_score','rowID','Enzyme ID','Enzyme Name','Subsystems','Direction','Gr Rule');
else              %Output to screen
    fprintf(formatSpec,'Slope','K_score','rowID','Enzyme ID','Enzyme Name','Subsystems','Direction','Gr Rule');
end

for num=1:length(fseof.target)
    if fseof.target(num,1) == 1
        A0=num2str((fseof.results(num,iterations)-fseof.results(num,1))/abs(targetMax-targetMax/iterations)); %Slope calculation
        A01 = num2str((fseof.results(num,iterations)-fseof.results(num,1))/abs(fseof.results(num,1))); %Slope calculation
        ...
        if output == 1    %Output to a file
            fprintf(fid,formatSpec,A0,A01,A1,A2,A3,A4,A5,A6);
        else              %Output screen
            fprintf(formatSpec,A0,A01,A1,A2,A3,A4,A5,A6);
        end
    end
end
