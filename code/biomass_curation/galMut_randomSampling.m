...
%modelConst = setParam(GALmutant,'obj',bio_pos,1);
%modelConst = changeMedia_batch(modelConst,'galactitol exchange',1);
%model = modelConst;
model = changeMedia_batch(GALmutant,'lactose exchange',1);
nSamples = 10000;
...
writetable(fluxTable,'../../results/randomSampling_GALmutant_lactose.txt','delimiter','\t','QuoteStrings',false)
...
writetable(allOxRed_solutions,'../../results/randomSampling_GALmutant_lactose_oxredGenes.txt','delimiter','\t','QuoteStrings',false)
...
writetable(sorbose_solutions,'../../results/sorbose_solutions_galMut.txt','delimiter','\t','QuoteStrings',false)
sorbose_gene_table = table(model.genes,model.geneShortNames,model.proteins,newMat(:,indexes));
%get sorbose gene table 
sorb_gene_mat = [];
indexes = find(solutions(x,:)>=flux_threshold);
x = find(strcmp(model.rxns,'r_4041'));
x = find(solutions(x,:)>=flux_threshold);
indexes = [indexes,x];

sorbose_gene_table = table(model.genes,model.geneShortNames,model.proteins,newMat(:,indexes));
writetable(sorbose_gene_table,'../../results/sorbose_gene_table_galMut.txt','delimiter','\t','QuoteStrings',false)
