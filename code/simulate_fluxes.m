...
for i=1:iterations
    ...
    if ~isempty(solWT.x) & ~isempty(solMT.x)
        ...
        exchangesMT(i,:) = solMT.x(indexes)';
    end
end
%Visualize results for mutant model
matrix = abs(exchangesMT);
fig = plot(matrix(:,2),matrix(:,3),matrix(:,2),matrix(:,4),matrix(:,2),matrix(:,5),matrix(:,2),matrix(:,6),'LineWidth',4);
...
saveas(fig,['../results/figures/exch_fluxes_WT.fig'])
saveas(fig,['../results/figures/exch_fluxes_WT.pdf'])
