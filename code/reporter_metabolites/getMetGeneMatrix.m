function [MetGeneMatrix,metsConectivity,genesConectivity] = getMetGeneMatrix(model,genes)
%getGeneMetMatrix
%   
...
MetGeneMatrix = abs(sign(Smat))*RGmat;
MetGeneMatrix = full(MetGeneMatrix);
%Calculate row and column sums
metsConectivity  = table(model.metNames(MetsIndxs),model.mets(MetsIndxs),model.metComps(MetsIndxs),sum(logical(MetGeneMatrix),2),'VariableNames',{'metNames' 'mets' 'metComps' 'genes_number'});
genesConectivity = table(model.genes(iB),model.geneShortNames(iB),sum(logical(MetGeneMatrix),1)','VariableNames',{'genes' 'short_names' 'mets_number'});
end
