function repMets=get_repMets(model,genes,genePValues,toExclude,printResults,outputFile,geneFoldChanges)
% reporterMetabolites
...
if ~isempty(toExclude)
    for met=toExclude
        idxs = find(strcmpi(metNames,met));
        metNames(idxs) = [];
        mets(idxs) = [];
        metZScores(idxs) = [];
        metPValues(idxs) = [];
...
repMets(1).test='all';
if any(geneFoldChanges)
    repMets=[repMets;get_repMets(model,genes(geneFoldChanges>=0),genePValues(geneFoldChanges>=0),toExclude)];
    repMets=[repMets;get_repMets(model,genes(geneFoldChanges<0),genePValues(geneFoldChanges<0),toExclude)];
...
if printResults==true
    for i=1:numel(repMets)
        fprintf(['TOP 20 REPORTER METABOLITES\nTEST TYPE: ' repMets(i).test '\nID\tNAME\tCOMPARTMENT\tP-VALUE\n']);
        for j=1:min(20,numel(repMets(i).mets))
            idx = find(strcmp(model.mets,repMets(i).mets{j}));
            compartment = model.compNames(model.metComps(idx));
            fprintf([repMets(i).mets{j} '\t' repMets(i).metNames{j} '\t' compartment{1} '\t' num2str(repMets(i).metPValues(j)) '\n']);
        end
        fprintf('\n');
    end
end
...
if any(outputFile)
    metGeneMat = getMetGeneMatrix(model);
    fid=fopen(outputFile,'w');
    for i=1:numel(repMets)
        fprintf(fid,['REPORTER METABOLITES USING TEST TYPE: ' repMets(i).test '\n']);
        fprintf(fid,'ID\tNAME\tCOMPARTMENT\tZ-SCORE\tP-VALUE\tNUMBER OF NEIGHBOURS\tAVERAGE Z-SCORE\tSTD Z-SCORE\tGENES\n');
        for j=1:numel(repMets(i).mets)
            idx   = find(strcmp(model.mets,repMets(i).mets{j}));
            genes = model.genes(find(metGeneMat(idx,:)));
            genes = strjoin(genes,',');
            compartment = model.compNames(model.metComps(idx));
            fprintf(fid,[repMets(i).mets{j} '\t' repMets(i).metNames{j} '\t' compartment{1} '\t' num2str(repMets(i).metZScores(j)) '\t' num2str(repMets(i).metPValues(j)) '\t' num2str(repMets(i).metNGenes(j)) '\t' num2str(repMets(i).meanZ(j)) '\t' num2str(repMets(i).stdZ(j)) '\t' genes '\n']);
        end
    end
    fclose(fid);
end
