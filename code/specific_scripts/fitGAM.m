...
% Ivan Domenzain.   Last update: 2020-03-06
%

if isfile('../../databases/chemostatData.tsv')
    %Load chemostat data:
    ...
    %If chemostat data file is not available then return GAM from biomass
    %pseudoreaction
    cd ..
    parameters = getModelParameters;
    cd limit_proteins
    bioPos  = find(strcmp(model.rxns,parameters.bioRxn));
    bioPrec = find(model.S(:,bioPos));
    %Identify ATP 
    ATPindex = bioPrec(strcmpi(model.metNames(bioPrec),'ATP'));
    %Extract GAM 
    GAM = abs(model.S(ATPindex,bioPos));
end
end
